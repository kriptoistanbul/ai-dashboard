...
def prepare_data(df):
    ...
    # Add domain column if available
    if 'Results' in df.columns:
    ...
    # Add date column (without time) based on the 'Time' column
...
# Function to safely get the date range from a DataFrame
...
# Load data from Google Sheet with caching for 1 hour
...
# Apply a date range filter to the DataFrame
...
# Apply a position filter to the DataFrame
def apply_position_filter(df, position_min=None, position_max=None):
    """Filter the DataFrame by position range."""
...
# Apply a keyword filter to the DataFrame
...
# Apply a domain filter to the DataFrame
def apply_domain_filter(df, domain):
    """Filter the DataFrame by domain."""
    ...
    if df is not None:
        ...
        with tabs[4]:
            ...
            if 'Keyword' in df.columns:
                ...
                with col1:
                    keywords = sorted(df['Keyword'].unique().tolist())
                    time_compare_keyword = st.selectbox("Select Keyword", ["-- Select a keyword --"] + keywords, key="time_comparison_keyword")
                ...
                if time_compare_keyword != "-- Select a keyword --":
                    ...
                    if not keyword_df.empty:
                        ...
                        if date_column:
                            ...
                            if date_strings:
                                with col2:
                                    start_date = st.selectbox("Start Date", ["-- Select start date --"] + date_strings, key="time_comparison_start_date")
                                with col3:
                                    end_date = st.selectbox("End Date", ["-- Select end date --"] + date_strings, key="time_comparison_end_date")
