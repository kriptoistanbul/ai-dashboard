        ...
        with tabs[0]:
            ...
            if filtered_df.empty:
                ...
                if 'Position' in filtered_df.columns:
                    ...
                    fig = px.histogram(
                        filtered_df[filtered_df['Position'] <= top_rank], 
                        x='Position',
                        title=f'Position Distribution (Top {top_rank})',
                        nbins=top_rank,
        ...
        with tabs[1]:
            ...
            if 'Keyword' in df.columns:
                ...
                if selected_keyword != "-- Select a keyword --":
                    ...
                    if keyword_df.empty:
                        st.warning("No data available for the selected keyword and filters.")
                    else:
        ...
        with tabs[2]:
            ...
            if domain_input and analyze_domain:
                ...
                if domain_df.empty:
                    ...
                    if domain_df.empty:
                        st.warning("No data available for the selected domain and filters.")
                    else:
                            ...
                            if date_strings:
                                ...
                                if start_date != "-- Select start date --" and end_date != "-- Select end date --" and compare_time:
                                    ...
                                    if date_column == 'date':
                                        ...
                                        # If using 'Time', filter by date part
                                        start_data = keyword_df[keyword_df['Time'].dt.date == start_date_obj].copy()
                                        end_data = keyword_df[keyword_df['Time'].dt.date == end_date_obj].copy()
                                    
                                    if start_data.empty:
                                        st.warning(f"No data found for start date: {start_date}")
                                    if end_data.empty:
                                        st.warning(f"No data found for end date: {end_date}")
