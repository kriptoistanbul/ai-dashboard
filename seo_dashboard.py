...
def compare_over_time():
    try:
        ...
        # Normalize column names - handle both letter columns (C, D, E) and named columns
        if 'C' in df.columns and 'D' in df.columns and 'E' in df.columns:
            ...
            if 'F' in df.columns:
                df = df.rename(columns={'F': 'Time'})
            elif 'B' in df.columns:
                df = df.rename(columns={'B': 'Time'})
        
        # Ensure we have required columns
        required_cols = ['Results', 'Position', 'Keyword']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            return jsonify({'error': f'Required columns {missing_cols} not found in data'})
        ...
        # Filter by keyword
        print(f"Filtering by {keyword_column} = '{keyword}'")
        keyword_df = df[df[keyword_column] == keyword].copy()
        ...
        # Convert to datetime
        keyword_df['datetime'] = pd.to_datetime(keyword_df[date_col], errors='coerce')
        
        # Create a string formatted date for matching
        keyword_df['date_str'] = keyword_df['datetime'].dt.strftime('%Y-%m-%d')
        
        # Get unique dates for this keyword
        unique_dates = sorted(keyword_df['date_str'].dropna().unique().tolist())
        print(f"Available dates for '{keyword}': {unique_dates}")
        
        # Try to parse and match the input dates
        try:
        ...
        start_matches = [d for d in unique_dates if formatted_start in d]
        end_matches = [d for d in unique_dates if formatted_end in d]
        
        # If no matches, use closest dates
        if not start_matches and unique_dates:
            start_matches = [unique_dates[0]]
        if not end_matches and unique_dates:
            end_matches = [unique_dates[-1]]
        
        print(f"Start date matches: {start_matches}")
        print(f"End date matches: {end_matches}")
        
        # Filter data for matched dates
        start_data = keyword_df[keyword_df['date_str'].isin(start_matches)]
        end_data = keyword_df[keyword_df['date_str'].isin(end_matches)]
        
        # If we have no data, return error
        if start_data.empty and end_data.empty:
            return jsonify({'error': f'No data found for dates: {start_date_str} and {end_date_str}'})
        
        print(f"Got {len(start_data)} rows for start date and {len(end_data)} rows for end date")
        
        # CRITICAL FIX: Make sure we don't accidentally filter out data
        # Sort by Position but keep ALL results
        
        # Prepare start date data
        start_urls = []
        if not start_data.empty:
            # Remove duplicates by URL if needed - keep the best position for each URL
            if start_data.duplicated('Results').any():
                start_data = start_data.sort_values('Position').drop_duplicates('Results')
                
            # Sort by Position (best positions first)
            start_data_sorted = start_data.sort_values('Position', ascending=True)
            
            # Create URL to position mapping for end data (for change calculations)
            end_positions = {}
            if not end_data.empty:
                for idx, row in end_data.iterrows():
                    url = row['Results']
                    pos = row['Position']
                    if pd.notna(url) and pd.notna(pos):
                        try:
                            end_positions[url] = int(pos) if isinstance(pos, (int, float)) else pos
                        except:
                            pass
        ...
        if not end_data.empty:
            # Remove duplicates by URL if needed - keep the best position for each URL
            if end_data.duplicated('Results').any():
                end_data = end_data.sort_values('Position').drop_duplicates('Results')
                
            # Sort by Position (best positions first)
            end_data_sorted = end_data.sort_values('Position', ascending=True)
            
            # Create URL to position mapping for start data (for change calculations)
            start_positions = {}
            if not start_data.empty:
                for idx, row in start_data.iterrows():
                    ...
                    if pd.notna(url) and pd.notna(pos):
                        try:
                            start_positions[url] = int(pos) if isinstance(pos, (int, float)) else pos
                        except:
                            pass
        ...
        # Build position changes data
        for url in all_urls:
            start_pos = start_pos_map.get(url)
            end_pos = end_pos_map.get(url)
        ...
        # Sort position changes by status and change magnitude
        ...
        print(f"Returning data with {len(start_urls)} start URLs and {len(end_urls)} end URLs")
